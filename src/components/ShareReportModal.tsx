...
export function ShareReportModal({ open, onClose, variant, data }: Props) {
  ...
  const buildEmailParts = (mode: "html" | "text" = "text") => {
    ...
    const sectionStyle =
      "margin:18px 0 6px;font-size:14px;font-weight:700;letter-spacing:.04em;color:#191c1e;text-transform:uppercase;";
    const hrStyle = "border:none;border-top:1px solid #cbd5e1;margin:0 0 10px;";
    const rowStyle = "margin:3px 0;font-size:15px;line-height:1.5;color:#191c1e;";
    ...
    const bodyHTML = `
<div style="font-family:Arial,Helvetica,sans-serif;color:#191c1e;max-width:680px;">
  ...
  <p style="${rowStyle}"><span style="${labelStyle}">Link del reporte:</span> <a href="${link}" style="color:#005da9;">${link}</a></p>
</div>`.trim();
