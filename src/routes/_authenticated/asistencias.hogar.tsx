...
export const Route = createFileRoute("/_authenticated/asistencias/hogar")({
  head: () => ({
    meta: [
      ...
      { name: "description", content: "Registrar nueva asistencia para el hogar." },
    ],
  }),
      ...
      <main className="flex-1 w-full max-w-7xl mx-auto px-6 py-8">
        <header className="mb-8">
          <div className="flex items-center gap-2 text-[#005da9] font-body-bold text-body-bold mb-1">
            <span className="material-symbols-outlined" style={{ fontVariationSettings: '"FILL" 1', fontSize: "20px" }}>home</span>
            <span className="uppercase tracking-wider text-[10px]">Asistencias</span>
          </div>
          <h1 className="text-display-lg text-[#191c1e]">Registrar nueva asistencia para hogar</h1>
          <p className="text-body-lg text-[#414752]">Complete los campos para registrar una nueva asistencia para hogar.</p>
        </header>
        <AsistenciaCard />
      </main>
    </div>
        ...
        <div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-x-6 gap-y-4">
            ...
            <Field label="Tipo de asistencia" required>
              <SelectMasterList listKey="asist_hogar" value={form.tipoAsistencia ?? ""} onChange={(v) => set("tipoAsistencia", v)} placeholder="Seleccione el tipo de asistencia" />
            </Field>
            ...
            <Field label="Dirección donde se asiste" required>
              <input type="text" value={form.direccion ?? ""} onChange={(e) => set("direccion", e.target.value)} placeholder="Dirección donde se asiste" className="form-input" />
            </Field>
            <div className="md:col-span-2 space-y-1.5">
              <label className="text-label-sm text-[#414752]">Detalle del problema <span className="text-[#005da9]"> *</span></label>
              <textarea value={form.detalleProblema ?? ""} onChange={(e) => set("detalleProblema", e.target.value)} placeholder="Detalle del problema" rows={3} className="form-input" style={{ height: "auto", padding: "0.5rem 0.75rem" }} />
            </div>
          </div>
        </div>
