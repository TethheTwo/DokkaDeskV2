      ...
      <main className="flex-1 w-full max-w-7xl mx-auto px-6 py-8">
        <header className="mb-8">
          <div className="flex items-center gap-2 text-[#005da9] font-body-bold text-body-bold mb-1">
            <span className="material-symbols-outlined" style={{ fontVariationSettings: '"FILL" 1', fontSize: "20px" }}>directions_car</span>
            <span className="uppercase tracking-wider text-[10px]">Asistencias</span>
          </div>
          <h1 className="text-display-lg text-[#191c1e]">Registrar nueva asistencia para automotor</h1>
          <p className="text-body-lg text-[#414752]">Complete los campos para registrar una nueva asistencia para automotor.</p>
        </header>
        <AsistenciaCard />
      </main>
    </div>
    ...
    <form onSubmit={handleSubmit} className="bg-white rounded-xl border border-[#e2e8f0] shadow-sm p-6 md:p-8 space-y-8">
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-x-8 gap-y-8">
        <div>
          ...
          <div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-x-6 gap-y-4">
              ...
              <Field label="Tipo de asistencia" required>
                <SelectMasterList listKey="asist_automotor" value={form.tipoAsistencia ?? ""} onChange={(v) => set("tipoAsistencia", v)} placeholder="Seleccione el tipo de asistencia" />
              </Field>
              ...
              <Field label="Dirección donde se asiste" required>
                <input type="text" value={form.direccionAsiste ?? ""} onChange={(e) => set("direccionAsiste", e.target.value)} placeholder="Dirección donde se asiste" className="form-input" />
              </Field>
              <Field label="Dirección donde se llevará">
                <input type="text" value={form.direccionLleva ?? ""} onChange={(e) => set("direccionLleva", e.target.value)} placeholder="Dirección donde se llevará" className="form-input" />
              </Field>
            </div>
          </div>
          ...
          <div>
            <div className="flex items-center gap-2 mb-4 pb-2 border-b border-[#e2e8f0]">
              <span className="material-symbols-outlined text-[#005da9] text-[20px]">directions_car</span>
              <h3 className="text-body-bold text-[#191c1e] uppercase tracking-wider">Información del vehículo</h3>
            </div>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-x-6 gap-y-4">
              <Field label="Placa" required>
                <input type="text" value={form.placa ?? ""} onChange={(e) => set("placa", e.target.value)} placeholder="Placa del vehículo" className="form-input" />
              </Field>
              <Field label="Marca" required>
                <input type="text" value={form.marca ?? ""} onChange={(e) => set("marca", e.target.value)} placeholder="Marca del vehículo" className="form-input" />
              </Field>
              <Field label="Modelo" required>
                <input type="text" value={form.modelo ?? ""} onChange={(e) => set("modelo", e.target.value)} placeholder="Modelo del vehículo" className="form-input" />
              </Field>
              ...
              <div className="flex flex-wrap gap-2">
                {attachments.map((f, i) => (
                  <div key={i} className="flex items-center gap-2 bg-[#f2f3f6] rounded-md px-3 py-1.5 text-sm">
                    ...
                    <button type="button" onClick={() => setAttachments(attachments.filter((_, idx) => idx !== i))} className="text-[#ba1a1a] hover:text-[#93000a]">
